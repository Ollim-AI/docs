...
how routines, reminders, chains, and embeds compose into a daily system.
...
<Tip>
  Every routine here is `background: true`. The agent works silently
  and only reaches out via `discord_embed` or `ping_user` when it has
  something worth saying. Some routines (like identity stabilize) never
  ping at all.
</Tip>
...
```bash
ollim-bot reminder add --delay <mins> --background --max-chain 3 \
  -m "<prompt>" -d "Nightly sleep chain"
```
