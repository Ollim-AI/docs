...
- **`update_main_session: "freely"`** — reports updates only when
...
from natural language requests. When you describe what you want, the
bot engineers the entire solution: YAML configuration, prompt logic,
button interactions, follow-up chains, and cross-routine coordination.
...
From that single sentence, the bot:
...
4. **Encoded a complete program** in the routine prompt:
...
shown above — a routine that runs daily and adapts to weather,
calendar, recent workout history, and real-time user feedback through
buttons.
...
<Tip>
You don't need to understand how to write routines. Describe what you
want — "remind me to stretch every 2 hours but not during meetings" —
and the bot handles the implementation. The examples on this page show
what the bot produces, not what you need to write.
</Tip>
