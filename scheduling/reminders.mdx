...
| `chain_parent` | `str \| null` | `null` | ID of the chain root |
...
| `allowed_tools` | `list[str]` | `null` | MCP tool allowlist (bg only) |
...
- `chain_depth` reaches `max_chain` — the agent receives an error with
  recovery guidance: "follow-up limit reached — this was the last check.
...
increments `chain_depth`. Chain reminders also inherit tool restrictions and skills
