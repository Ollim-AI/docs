...
<Tabs>
  <Tab title="CLI">
    ...
    **Add a reminder:**
    ...
    Reminders run in the background by default. These flags configure background behavior:

    | Flag | Description |
    |------|-------------|
    | `--foreground` | Run in the foreground instead of a background fork |
    ...
    **List pending reminders:**
    ...
    ```text
      f4a9c1e2  at 2026-02-24T15:30  Check deployment status
      b7d3e8a1  at 2026-02-24T16:00  (chain 1/3)  Check for email response
    ```
...
Set `background: false` (or pass `--foreground` via CLI) to run the
reminder in the foreground instead. The agent sends you a DM with its
response, just like a normal conversation.
