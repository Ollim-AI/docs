...
`ping_user` is **background-fork-only** — it sends a plain text alert
and is not available in the main session or interactive forks.
`discord_embed` is **available in all contexts** — main session,
interactive forks, and background forks alike.
...
When `update_main_session` is `blocked` and `allow_ping` is `true`, the
...
<Note>
When `allowed_tools` is not set, background forks receive the `MINIMAL_BG_TOOLS`
default — just enough for reporting and notification:
...
`tool_policy.py` validates all `allowed_tools` patterns at startup.
...
If a pattern fails validation, the bot logs an error at startup. This
catches misconfigurations before they reach the agent.
