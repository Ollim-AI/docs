...
| Field | Default | Description |
|-------|---------|-------------|
| `allowed-tools` | Minimal defaults | Allowlist — overrides defaults when set. |

<Note>
When `allowed-tools` is not set, background forks receive the `MINIMAL_BG_TOOLS`
...
| `allowed-tools` | `list[string]` | — | Tool allowlist (SDK format) |
