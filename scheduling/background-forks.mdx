...
<Note>
When you're mid-conversation (the agent lock is held), background forks
enter "quiet mode." Non-critical pings return an error suggesting the agent
use `report_updates` instead. Set `critical: true` on a ping to bypass this.
</Note>
