...
| `blocked` | Reporting disabled | `report_updates` and `follow_up_chain` stripped at SDK level |
...
| Field | Default | Description |
|-------|---------|-------------|
| `allowed-tools` | Minimal defaults | Additional tools merged with minimal defaults. |

<Note>
Every background fork always receives **MINIMAL_BG_TOOLS** — the communication
channel back to you. These tools cannot be removed:
...
When you set `allowed-tools`, your tools are **merged in** on top of these
defaults. Duplicates are dropped, so listing a system tool explicitly has
no effect. You cannot accidentally disable reporting or pinging by
forgetting to include a system tool.
</Note>
...
`tool_policy.py` validates all `allowed-tools` patterns at startup.
...
| `allowed-tools` | `list[string]` | — | Additional tools merged with minimal defaults (SDK format) |
