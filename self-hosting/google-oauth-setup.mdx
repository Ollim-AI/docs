...
<Steps>
  ...
  <Step title="Save credentials to the data directory">
    Move the downloaded file to `~/.ollim-bot/credentials.json`:

    ```bash
    mkdir -p ~/.ollim-bot
    mv ~/Downloads/client_secret_*.json ~/.ollim-bot/credentials.json
    ```
    ...
    ```
    Missing ~/.ollim-bot/credentials.json -- download OAuth client credentials
  ...
  <Step title="Complete the first authentication flow">
    ...
    1. Reads `~/.ollim-bot/credentials.json`
    ...
    6. Saves the tokens to `~/.ollim-bot/token.json`
...
The token file at `~/.ollim-bot/token.json` stores the access token,
...
<Columns cols={2}>
  ...
  <Card title="Setup guide" icon="book" href="/getting-started/setup">
    Full setup walkthrough including Discord and all integrations.
  </Card>
