...
<Steps>
  ...
  <Step title="Set up Google OAuth (optional)">
    ...
    authorized token is saved to `~/.ollim-bot/state/token.json` and reused automatically.
  ...
  <Step title="Run the bot">
    ...
    2. Checks that no other instance is running (PID file at `~/.ollim-bot/state/bot.pid`)
    ...
    <Note>
      ...
      cleaning up `~/.ollim-bot/state/bot.pid`, delete the file manually before restarting.
    </Note>
  </Step>
</Steps>
...
ollim-bot persists its Claude session ID to `~/.ollim-bot/state/sessions.json`.
...
`~/.ollim-bot/state/session_history.jsonl` for debugging. See
