...
<AccordionGroup>
  <Accordion title="Ollama">
    ...
    Install Ollama, pull a model, and verify the server is running:
    ...
    smaller ones. Local inference is still significantly slower than
  ...
  <Accordion title="LiteLLM proxy">
    ...
    route through a [LiteLLM proxy](https://docs.litellm.ai/)
    (v1.81+) — it translates to Anthropic format automatically and
    adds cost tracking, load balancing, and guardrails.
  </Accordion>
</AccordionGroup>
...
<Warning>
  Self-hosting requires a dedicated GPU with at least 24GB VRAM. Consumer
  GPUs (8–16GB) cannot run the models listed below at sufficient quality for
  reliable tool calling. If you don't have the hardware, consider
  [alternative subscriptions](#alternative-subscriptions) or
...
| Model | Active params | Hardware | Tool use | License |
|-------|--------------|----------|----------|---------|
| **Qwen3-Coder-Next** | 3B (80B MoE) | 32GB+ VRAM | Excellent — SWE-Bench 70.6 | Apache 2.0 |
| **Qwen3.5-35B-A3B** | 3B (35B MoE) | 32GB VRAM | Good — 1M context | Apache 2.0 |
| **GLM-4.7-Flash** | 30B | 24GB+ VRAM | Good — interleaved reasoning | Open weights |
| **DeepSeek V3.2 (32B distill)** | 32B | 24GB+ VRAM | Good — thinking + tool use | MIT |
...
<Tip>
  ...
  Tool use in local models has matured significantly in early 2026 —
