...
dataclasses, idle timeouts) — keeping each file under 400 lines.
...
`_chain_context_var` and `_msg_collector` in `agent_tools.py` follow the
same pattern. This lets multiple forks run concurrently while the main
session uses module-level globals for the same values.
...
The codebase has 38 modules organized into five layers.

<Accordion title="Full module map (38 modules)">
