...
The codebase has 21 modules organized into five layers.
...
| `bot.py` | Discord interface — DMs, @mentions, slash commands, reaction acks |
...
| `views.py` | Persistent button handlers via `DynamicItem` — delegates to google/, forks |
...
<Steps>
  <Step title="Discord event">
    `bot.py` receives a DM or @mention. It extracts text and image
...
`~/.ollim-bot/pending_updates.json`. The main session pops these
...
via `contextvars` so concurrent forks don't interfere with each other
or the main session.
</Note>
...
persisted to `~/.ollim-bot/sessions.json`. On restart, it resumes the
existing session. All session lifecycle events (created, compacted,
swapped, cleared, forked) are logged to `session_history.jsonl`.

### Contextvar isolation

Background forks use `ContextVar` instances to scope mutable state:
`_in_fork_var`, `_busy_var`, `_channel_var`, `_chain_context_var`,
and `_bg_fork_config_var`. This lets multiple forks run concurrently
while the main session uses module-level globals for the same values.
...
- **JSONL** for append-only logs (session history, pending updates)
