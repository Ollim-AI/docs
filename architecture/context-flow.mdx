...
3. **Background forks** branch from main (or start fresh if isolated,
   or resume from a saved session if persistent), receive a preamble
   with budget and schedule context
...
<Tabs>
  ...
  <Tab title="Persistent background fork">
    Persistent routine sessions (`session: persistent`) do **not**
    receive pending updates. Like isolated forks, they start from
    their own standalone context — but unlike isolated forks, they
    resume from their saved session rather than starting fresh.
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Persistent session">
    Included only when `session: persistent` is set. Tells the agent
    that its context carries across fires and that it has a
    `compact_session` tool available for compressing context when it
    grows large.
  </Accordion>
