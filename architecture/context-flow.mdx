...
When `blocked` is combined with `allow_ping: true`, the agent is told
that no summary reaches the main session, but it can still ping the
user directly on Discord for time-sensitive items.

A stop hook (`require_report_hook`) enforces these modes. In `always`
mode, the hook prompts the agent to summarize what it found or did to
update the main session. In `on_ping` mode, it blocks if the agent
pinged but didn't report.

### Read path

The main session and forks read pending updates differently. The
injected header also varies by context — the main session sees
"RECENT BACKGROUND UPDATES (mention key findings in your response)"
while forks see "RECENT BACKGROUND UPDATES (read-only -- main session
will also see these)".
...
<Tabs>
  <Tab title="Main session">
    ...
    The header instructs the agent to mention key findings in its
    response.
  ...
  <Tab title="Interactive fork">
    ...
    consume it. The header marks these as read-only so the agent
    knows the main session will also receive them.
  </Tab>
...
- On the final check: `follow_up_chain` returns an error with recovery
  guidance — "follow-up limit reached -- this was the last check. If the
  task still needs attention, ping the user now." This prompts the agent
  to escalate via `ping_user` rather than silently dropping the task.
