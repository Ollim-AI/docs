...
- On the final check: the preamble warns that `follow_up_chain` is not
  available and this is the last chance to act. If the agent calls
  `follow_up_chain` anyway, the tool returns an error with recovery
  guidance prompting the agent to escalate via `ping_user`.
