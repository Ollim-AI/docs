...
The minimal background fork tool set (`MINIMAL_BG_TOOLS`) includes only the four discord MCP tools (`report_updates`, `ping_user`, `discord_embed`, `follow_up_chain`) and the `ollim-bot help` CLI command. Jobs can expand this via the `allowed-tools` YAML field — see [Background forks](/scheduling/background-forks#tool-restrictions).
