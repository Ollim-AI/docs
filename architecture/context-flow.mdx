...
| **MCP tools** | All 7 discord tools + docs server | Same as main | Discord tools dynamically gated | Discord tools dynamically gated |
...
The default background fork tool set (`DEFAULT_BG_TOOLS`) includes read-only file tools (`Read`, `Glob`, `Grep` scoped to `./**.md`), the `ollim-bot help` and `ollim-bot tasks *` CLI commands. Discord MCP tools (`ping_user`, `discord_embed`, `report_updates`, `follow_up_chain`) are not in this static list — they are dynamically gated at runtime based on the job's `allow-ping` and `update-main-session` flags. Jobs can expand the tool set via the `allowed-tools` YAML field — see [Background forks](/scheduling/background-forks#tool-restrictions).
