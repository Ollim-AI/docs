...
<AccordionGroup>
  ...
  <Accordion title="Skill instructions">
    ...
    more [skills](/extending/skills), the skill instructions are loaded
    and prepended to the job message at fire time. Tool dependencies
    declared in the skills (`allowed_tools`) are merged into the job's
    tool set via `collect_skill_tools()`. Skills propagate through
    chain reminders via `ChainContext`.
  </Accordion>
</AccordionGroup>
