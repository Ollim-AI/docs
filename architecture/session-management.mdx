...
| `persistent_bg` | Persistent routine session started or resumed (no parent session) |
...
Routines with `session: persistent` maintain their own standalone session
across fires. Unlike forked or isolated background sessions, persistent
sessions are not discarded — the session ID is saved to disk and resumed
on the next fire.

### Session storage

Each persistent routine stores its session ID as a plain string in
`~/.ollim-bot/state/routine_sessions/<routine-id>`. The file is created
after the first fire and updated after every subsequent fire (including
after deferred compaction).

| Function | Behavior |
|----------|----------|
| `load_persistent_session(routine_id)` | Returns the session ID string, or `None` if no file exists |
| `save_persistent_session(routine_id, session_id)` | Atomic write via tempfile + `os.replace` |
| `delete_persistent_session(routine_id)` | Removes the session file. Called by `sync_all()` when a routine is removed |

### Skip guard

A module-level set (`_active_persistent`) prevents concurrent fires
of the same persistent routine. If a persistent routine fires while
its previous fire is still running, the new fire is silently skipped.
The guard is released in the `finally` block of `run_agent_background`.

### Deferred compaction

Persistent sessions support agent-driven compaction via the
`compact_session` MCP tool. When the agent calls this tool, the
compaction instructions are stored in a contextvar and executed
**after** the agent finishes responding. This avoids interrupting the
agent mid-response. After compaction, the new session ID is saved.
...
| `~/.ollim-bot/state/routine_sessions/<id>` | Plain string | Persistent routine session ID (one file per routine) |
