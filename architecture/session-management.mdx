...
- **Automatic**: The SDK may auto-compact during normal conversation.
  When this happens, the CLI sends a `compact_boundary` message and then
  waits for a new query — it does not auto-continue. ollim-bot detects
  this and re-sends the user's original message against the freshly
  compacted context, so the agent produces a response without the user
  needing to repeat themselves. `save_session_id()` detects the session
  change by comparing the new session ID against the one on disk — if
  they differ, it logs a `compacted` event with the old ID as
  `parent_session_id`.
...
| `lookup_fork_session(message_id)` | Returns a `ForkLookup(session_id, expired)` named tuple |

`lookup_fork_session` returns one of three results for a given message ID:

| `session_id` | `expired` | Meaning |
|---|---|---|
| `str` | `False` | Live fork session — can resume as interactive fork |
| `None` | `True` | Record exists but past the 7-day TTL — bot signals expiry to the user |
| `None` | `False` | Unknown message ID — not a fork message |
