...
The main session ID is stored at `~/.ollim-bot/sessions.json`. Despite the
...
Every session transition is recorded in `~/.ollim-bot/session_history.jsonl`. Each line is a `SessionEvent`:
...
The Claude Agent SDK automatically compacts conversations when they grow
too long. From ollim-bot's perspective, compaction manifests as a changed
session ID — the SDK returns a new ID pointing to a compacted version of
the conversation.

`save_session_id()` detects this: if a session ID already exists on disk
and the new ID differs, it logs a `compacted` event with
`parent_session_id` set to the old ID, then writes the new ID. No explicit
compaction logic exists in ollim-bot — the SDK handles it transparently.
...
<Steps>
  ...
  <Step title="Delete the session file">
    Removes `~/.ollim-bot/sessions.json`. The next message will start a fresh session.
  </Step>
</Steps>
...
The mapping is stored in `~/.ollim-bot/fork_messages.json` as a JSON array of records:
...
| File | Format | Purpose |
|------|--------|---------|
| `~/.ollim-bot/sessions.json` | Plain string | Current main session ID |
| `~/.ollim-bot/session_history.jsonl` | JSONL (`SessionEvent`) | Append-only lifecycle event log |
| `~/.ollim-bot/fork_messages.json` | JSON array | Discord message to fork session mapping |
