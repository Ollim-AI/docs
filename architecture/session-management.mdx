...
When a background fork sends messages to Discord, the bot tracks which fork
sent which message. This enables the reply-to-fork feature — replying to a
background fork's message resumes that fork's conversation as an
interactive fork.
