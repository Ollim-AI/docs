...
1. The SDK emits a `SystemMessage` with `subtype="compact_boundary"`. The
   agent layer detects this and signals the streamer with a `StreamStatus`
   event (`kind="compact_start"`) including an optional `compact_tokens` count.
...
4. When compaction completes, the streamer edits the status to a permanent
