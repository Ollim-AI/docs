...
Tool labels are rendered progressively as subdued text in Discord (using
the `-# ` markdown small-text prefix). Each label flushes when the next
...
labels into `-# *{label}*` text — or
`-# *~~{label}~~ — denied (use /permissions ask to approve)*` if the
tool was denied by the permission system.
...
When the agent delegates work to a subagent (the `Agent` tool in Claude
...
```text
guide(search for docs) · Read... (12s)
```

The description is truncated to 40 characters. MCP tool names are
