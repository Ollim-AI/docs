...
1. `Agent.stream_chat()` yields text deltas and `StreamStatus` signals
   from the SDK's `StreamEvent` messages
...
phase transitions like thinking, tool use, and compaction). It delegates
raw SDK event parsing to `StreamParser` in `streamer.py`.
...
1. The SDK emits a `SystemMessage` with `subtype="compact_boundary"` and
   ends the stream. The agent layer detects this, extracts the
   pre-compaction token count from `compact_metadata`, and signals the
   streamer with a `StreamStatus` event (`kind="compact_start"`) including
...
4. The agent re-sends the original query against the freshly compacted
   context (via `client.query()`), then streams the new response
