...
<Note>
When an `enter_fork` tool fires during streaming, `stream_response()` in
`agent_streaming.py` interrupts the SDK client and suppresses remaining
...
1. The SDK emits a `SystemMessage` with `subtype="compact_boundary"` and
   ends the stream. `stream_response()` in `agent_streaming.py` detects
   this, extracts the pre-compaction token count from `compact_metadata`,
   and yields a `StreamStatus` event (`kind="compact_start"`) including
