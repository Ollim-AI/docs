...
- `credentials.json` in `~/.ollim-bot/` from the Google Cloud Console
- `token.json` in `~/.ollim-bot/` (auto-generated on first auth)
...
<Tabs>
  ...
  <Tab title="Agent">
    ...
    4. Reports findings in a structured format
    ...
    - **Financial** — bills due, failed payments, payment confirmations requiring review
...
<AccordionGroup>
  <Accordion title="Gmail commands return no messages">
    ...
    `~/.ollim-bot/token.json` and re-authenticate to pick up the new scope.
  </Accordion>
  ...
  <Accordion title="Dates display in Pacific Time">
    All Gmail timestamps are displayed in `America/Los_Angeles`
    (Pacific Time). This is currently not configurable.
  </Accordion>
</AccordionGroup>
