...
<Tabs>
  ...
  <Tab title="Agent">
    ...
    and the agent is informed of what you did so it stays in sync.
...
<Accordion title="Developer reference: API helpers">
  ...
  This is called via `asyncio.to_thread` in the button handler to avoid blocking the event loop.
</Accordion>
