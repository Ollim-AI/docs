...
<AccordionGroup>
  <Accordion title="Request processing pipeline">
    ...
    7. **Injection screening** — free-form text fields are checked for attempts to trick the bot (30-second timeout — if screening fails, the webhook proceeds). Flagged payloads are skipped and logged.
    8. **Validation** — if the spec references a `subagent` or `skills`, the bot verifies they exist. Missing references skip dispatch entirely.
