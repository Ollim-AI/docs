...
in a pending updates file (capped at **10 entries** — oldest dropped when
exceeded). When the user next sends a message, they are injected with a
...
and the header says "mention key findings in your response." In interactive
forks, they are peeked (read without clearing) and the header says
"read-only — main session will also see these."
...
| `blocked` + `allow_ping` | No summary to the main session, but can still ping the user directly for time-sensitive items |
