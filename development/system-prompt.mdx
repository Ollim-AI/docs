...
`load_profile()` in `profile.py` reads both files and concatenates
them with a blank line separator. The combined content is prepended
to the operational instructions.

`IDENTITY.md` is bootstrapped from a template on first run.
`{USER_NAME}` is substituted into the file at bootstrap time (baked
in), not at prompt-build time. `USER.md` is user-created — not
bootstrapped. Both files are readable and editable by the agent at
