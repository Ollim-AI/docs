...
The system prompt is built dynamically by `build_system_prompt()` in
`prompts.py` and passed to `ClaudeAgentOptions` in `agent.py`. It
composes two layers: user-editable profile files (`IDENTITY.md` and
`USER.md`) prepended to operational instructions covering behavior
rules and inline documentation for every tool. Additional context —
timestamps, pending background updates, and background preambles — is
injected at runtime per message.

## Prompt structure

`build_system_prompt()` concatenates the profile and operational
sections, in order:
...
<Note>
The operational section uses f-string substitution for two values:
`USER_NAME` (fork instructions, tool sections) and `TZ` (Google
Calendar section), both from `config.py`. The profile section is
loaded verbatim from disk — no template substitution at prompt-build
time.
</Note>

## Profile section

The profile is prepended to the operational instructions if either
`IDENTITY.md` or `USER.md` exists in `~/.ollim-bot/`. Both files are
loaded by `load_profile()` in `profile.py` and concatenated with a
blank line separator.

`IDENTITY.md` is bootstrapped on first run with a default template.
The template defines the bot's persona — tone, ADHD-awareness,
communication style, and decision guidance. `{USER_NAME}` is
substituted into the file content at bootstrap time (baked into the
file), not at prompt-build time. The default template includes:
...
Your output becomes conversation history you'll reason over later --
keep it tight. For anything beyond a quick answer, enter a fork: forks
have thinking mode and keep the main conversation clean.

## When {USER_NAME} asks what to do

- Consider deadlines and priorities.
- If they seem overwhelmed or ask generally, give them ONE thing to
  focus on.
- If they ask for a list or overview, give it.
```

`USER.md` is not bootstrapped — you create it yourself to provide
persistent context about your work patterns, schedule, ADHD specifics,
or priorities. Both files are editable by you and by the agent at
runtime. The operational section includes a note telling the agent
that these files exist and can be read or edited.

See [Data directory](/configuration/data-directory#profile-files)
for file locations and bootstrapping details.
...
<Tabs>
  ...
  <Tab title="Reminders">
    ...
    `update-main-session`, `allow-ping`), the agent enters a fork
...
User messages are not passed to the agent verbatim. The `prepend_context`
function in `agent_context.py` prepends two pieces of dynamic context before
every message:
...
sends a message, they are injected with a context-specific header:
...
In the main session, pending updates are consumed (cleared after
injection). In interactive forks, they are peeked (read without
clearing). See [Context flow](/architecture/context-flow#read-path) for
the full read path and header variants.

### Skills section

The system prompt's skills section tells the agent how skills work — the
directory structure, the `skills:` field on routines and reminders, and
how to invoke skills interactively using the SDK's built-in `Skill` tool.
The SDK discovers available skills natively via the
`.claude/skills/ -> ../skills/` symlink, so no custom index is appended
...
Each [subagent](/extending/subagents) has its own system prompt — a standalone
markdown file with YAML frontmatter in `subagents/`. These are
not derived from `build_system_prompt()`. Each file contains task-specific
