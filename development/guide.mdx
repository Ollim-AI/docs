...
<Steps>
  ...
  <Step title="Run the bot">
    ...
    On first run, the bot checks Claude authentication — if not logged in,
    it DMs you an OAuth link via Discord. See the
    [quickstart](/getting-started/quickstart) for the full first-run flow.
...
├── tool_policy.py       # Tool-pattern validation and SDK ceiling builder
...
| `claude-history` | Session transcript search — used by [subagents](/extending/subagents) to review past conversations |
