...
The bot requires the **Message Content Intent** to read message text. See the
[Discord bot setup guide](/self-hosting/discord-bot-setup#enable-the-message-content-intent)
for where to find this toggle.
...
<AccordionGroup>
  ...
  <Accordion title="Expired or invalid token">
    ...
    revocation), the bot
    [detects it automatically](/self-hosting/google-oauth-setup#auto-recovery)
    and sends you a message to reconnect. Run `/google-auth` to re-authenticate.
