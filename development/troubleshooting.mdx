...
The bot writes its PID to `~/.ollim-bot/bot.pid` on startup and checks
...
the webhook server. This typically means the Discord application has no owner
set — check the application settings in the
...
<AccordionGroup>
  ...
  <Accordion title="Recovering session history">
    The append-only log at `~/.ollim-bot/session_history.jsonl` records every
    lifecycle event. To inspect it:

    ```bash
    cat ~/.ollim-bot/session_history.jsonl
    ```
    ...
    ```bash
    echo -n "sess_abc123" > ~/.ollim-bot/sessions.json
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Corrupt JSONL files">
    `read_jsonl()` skips blank lines, non-JSON lines, and malformed entries.
    It also filters fields to only those known to the target dataclass,
    providing forward compatibility. Corrupt lines are silently dropped on
    read.
    ...
    ```bash
    rm ~/.ollim-bot/session_history.jsonl
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Missing credentials.json">
    The Google integration requires `~/.ollim-bot/credentials.json` from the
...
<AccordionGroup>
  ...
  <Accordion title="Ping budget exhausted">
    ...
    ```bash
    cat ~/.ollim-bot/ping_budget.json
    ```
...
<AccordionGroup>
  <Accordion title="Reply-to-fork not working">
    ...
    `~/.ollim-bot/fork_messages.json`, which maps Discord message IDs to
    fork session IDs.
...
|------|-------------------|
| `~/.ollim-bot/sessions.json` | Current session ID (or missing = no active session) |
| `~/.ollim-bot/session_history.jsonl` | Timeline of session lifecycle events |
| `~/.ollim-bot/bot.pid` | PID of running bot instance |
| `~/.ollim-bot/ping_budget.json` | Current ping budget state and counters |
| `~/.ollim-bot/pending_updates.json` | Queued background fork summaries |
| `~/.ollim-bot/fork_messages.json` | Discord message to fork session mappings |
| `~/.ollim-bot/inquiries.json` | Active button inquiry prompts |
