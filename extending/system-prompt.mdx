...
`build_skill_index()` also injects a dynamic skill index at `Agent.__init__`.
</Note>
...
The agent needs to know what skills are available so it can reference
them when creating routines. When skills exist in `~/.ollim-bot/skills/`,
`build_skill_index()` appends a dynamic index to the system prompt at
startup listing each skill's name and description.
