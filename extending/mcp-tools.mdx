...
ollim-bot exposes eight MCP tools to the agent via a server named `discord`. These tools let the agent send messages, manage forks, report findings, schedule follow-ups, and manage persistent session context — all without direct access to the Discord API.
...
Compact the current persistent session's context. Only available in [persistent routine sessions](/scheduling/routines#persistent-sessions).

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `instructions` | string | Yes | — | What to preserve and what to discard during compaction |

The agent calls this when its context is growing large. Compaction is **deferred** — it doesn't run immediately. Instead, the instructions are stored and compaction executes after the agent finishes responding. This ensures the agent's current response isn't interrupted.

Returns an error if called outside a persistent routine session.

**Availability:** Persistent routine sessions only. Automatically added to `allowed_tools` for persistent routines.

<Note>
The `compact_session` tool is separate from the `/compact` slash command. `/compact` compacts the main session interactively; `compact_session` compacts a persistent routine's standalone session with deferred execution.
</Note>
...
- The chain depth has reached `max_chain`
