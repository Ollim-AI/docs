...
The tool creates a new reminder via `ollim-bot reminder add` with the chain depth incremented by one. The tool forwards all configuration from the current `ChainContext`: `background`, `model`, `thinking`, `isolated`, `update-main-session`, `allow-ping`, `allowed-tools`, and `skills`.
...
Background forks use a stop hook (`require_report_hook`) that enforces the `update-main-session` policy before the agent can finish:
