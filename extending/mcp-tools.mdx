...
The tool creates a new reminder via `ollim-bot reminder add` with the chain depth incremented by one. All configuration from the current `ChainContext` is forwarded: `background`, `model`, `thinking`, `isolated`, `update_main_session`, `allow_ping`, `allowed_tools`, and `skills`.
