---
title: "Discord tools"
description: "Reference all seven Discord tools the agent uses for embeds, pings, forks, and chains."
---

ollim-bot gives the agent seven Discord tools for sending messages, managing forks, reporting findings, and scheduling follow-ups — all without direct access to the Discord API.
...
<Note>
Both output tools register the message so that replies are correctly routed back to the fork that sent them.
</Note>
...
The tool creates a new reminder via `ollim-bot reminder add` with the chain depth incremented by one. The tool forwards all configuration from the current chain: `background`, `model`, `thinking`, `isolated`, `update-main-session`, `allow-ping`, `allowed-tools`, and `skills`.
