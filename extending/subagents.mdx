...
|----------|---------|-------|-------|
| `guide` | Setup and usage help — surfaces relevant documentation verbatim | `haiku` | `mcp__docs__*`, `Read(**.md)`, `Glob(**.md)`, `Bash(ollim-bot help)`, `Bash(ollim-bot routine *)`, `Bash(ollim-bot reminder *)` |
...
<Note>
Each subagent only has access to the tools listed above. This follows the principle of least privilege — the guide cannot access session history, the gmail-reader cannot access session history, the history-reviewer cannot read email, and the user-proxy has read-only access.
</Note>
