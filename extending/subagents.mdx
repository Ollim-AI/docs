...
|----------|---------|-------|-------|
| `guide` | Setup and usage help — surfaces relevant documentation verbatim | `haiku` | `mcp__docs__*`, `WebFetch`, `Read(**.md)`, `Glob(**.md)`, `Bash(ollim-bot help)`, `Bash(ollim-bot routine list)`, `Bash(ollim-bot reminder list)` |
...
**Triggered by:** The main agent always delegates to the guide when the answer depends on ollim-bot documentation — setup, configuration, usage, YAML format, or feature behavior. This includes the user's own questions and when the agent itself needs docs knowledge (building a routine, checking webhook format, verifying how a feature works before using it).

**What it does:**
- Uses two search strategies based on question type: the `docs` MCP server for specific lookups (field names, YAML keys) and `WebFetch` for open-ended topics that need full-page context
...
Beyond the docs tools, the guide can read and glob `.md` files to inspect local configuration.
...
**Hooks:** Because `setting_sources=["project"]` is enabled, hooks configured in `~/.ollim-bot/.claude/settings.json` apply to the main agent session. Subagent-level hooks can also be defined in the `hooks` frontmatter field — they are scoped to that subagent's lifecycle. See the [Claude Code hooks guide](https://code.claude.com/docs/en/hooks-guide) for events, input schemas, and exit codes.
</Accordion>
