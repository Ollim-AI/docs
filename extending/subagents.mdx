...
Subagents are isolated agent instances that the main ollim-bot agent spawns to handle specialized tasks. Each subagent has its own system prompt, a restricted tool set, and a cheaper model — the main agent focuses on orchestration while subagents do the legwork.

## Overview

ollim-bot defines five subagents in `src/ollim_bot/subagents/`, each as a markdown file with YAML frontmatter. The main agent invokes them via the SDK's built-in `Task` tool. The main agent never performs subagent work directly — the system prompt explicitly instructs delegation.
...
Each `.md` file in `src/ollim_bot/subagents/` defines one subagent: YAML frontmatter for metadata, markdown body for the prompt. Here is the guide subagent as an example:
...
**Template variables:** `subagents.py` substitutes `{USER_NAME}` and `{BOT_NAME}` in the prompt and description at load time from `config.py`.
...
<Tabs>
  <Tab title="Guide">
    ...
    > When the user asks how to set up, configure, or use an ollim-bot feature, delegate to the guide subagent (via the Task tool). It searches docs.ollim.ai and returns the actual documentation text — prefer it over answering yourself, because paraphrasing docs introduces subtle errors. If the guide finds nothing, you can answer from code.
  </Tab>
  <Tab title="Gmail">
