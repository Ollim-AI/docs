...
- `load_agent_tool_sets()` reads YAML frontmatter from installed agents to extract tool declarations for [tool policy](/architecture/tool-policy) validation. Returns a mapping of `"subagent:name"` to tool list.

**Spec file format:** Each `.md` file is a raw markdown file with YAML frontmatter — no Python dataclass involved. The SDK reads these files directly.

| Frontmatter field | Description |
|-------------------|-------------|
| `name` | Subagent identifier (matches the filename without `.md`) |
| `description` | Used by the SDK to determine when/how to invoke the subagent |
| `model` | Model to use (`sonnet`, `haiku`, `opus`) |
| `tools` | Restricted tool access — YAML list or comma-separated string |

The markdown body below the frontmatter is the subagent's system prompt.

**Overrides:** Drop a `.md` file into `~/.ollim-bot/.claude/agents/` to override a built-in subagent (same filename) or add a new one. Since `install_agents()` skips existing files, your overrides persist across bot updates. The override file must be a complete spec — there is no partial merge.
</Accordion>
