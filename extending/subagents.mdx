...
**Runtime overrides and additions:** Drop a `.md` file into `~/.ollim-bot/subagents/` to override a built-in subagent (same `name` field) or add an entirely new one. Override files must be complete specs — there is no partial merge.
