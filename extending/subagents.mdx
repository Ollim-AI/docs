...
ollim-bot defines five subagents in `src/ollim_bot/subagents/`, each as a markdown file with YAML frontmatter. The SDK invokes them via the built-in `Task` tool. The main agent never performs subagent work directly — the system prompt explicitly instructs delegation.
