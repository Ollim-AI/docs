...
ollim-bot has seven extensibility mechanisms. Most are **file-based** — markdown files with YAML frontmatter that the agent creates and manages directly. No code changes required. Two mechanisms require **forking the repo** and editing Python source.
...
| **Subagents** | Delegated by main agent | Background fork | Email triage, history review, responsiveness analysis | Code |

### File-based mechanisms

Routines, reminders, webhooks, and skills are all markdown files stored in `~/.ollim-bot/`. The scheduler picks up changes every 10 seconds. You can create them by talking to the bot, using the CLI, or writing files directly.
...
Adding new MCP tools, subagents, Google services, or CLI commands requires modifying the Python source. Fork the repo, make your changes, and pull upstream updates when needed.
...
<Columns cols={2}>
  ...
  <Card title="Subagents" icon="robot" href="/extending/subagents">
    Five specialized agents for email triage, history review, and more.
  </Card>
...
<Note>
You can override a built-in subagent's prompt without code changes — drop a `.md` file with the same `name` field into `~/.ollim-bot/subagents/`. See [Subagents](/extending/subagents#developer-reference) for the spec format.
</Note>
