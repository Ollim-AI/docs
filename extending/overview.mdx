...
<Tabs>
  <Tab title="Routines vs Reminders">
    ...
    | **Follow-up** | Next occurrence is automatic | `max_chain` + `follow_up_chain` for progressive checks |
    ...
    Use **routines** for anything that repeats on a schedule. Use **reminders** for one-shot nudges — and set `max_chain` when the agent should follow up if the first nudge wasn't enough.
  </Tab>
...
| `allowed-tools` | list | — | Allowlist of tools — routines and reminders only (overrides defaults) |
