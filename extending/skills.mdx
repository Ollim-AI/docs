...
<Tip>
Keep `description` concise and action-oriented. It appears in the [system prompt's skill index](/extending/system-prompt#skill-index), so the agent uses it to decide when a skill is relevant — even outside of scheduled jobs.
</Tip>
...
Skills are loaded by `load_skills()` in the scheduler before being passed to this function.
</Accordion>
