...
| `allowed-tools` | No | Tools allowed without prompts when this skill runs. In the **scheduler context**, also merged into the routine or reminder's tool set via `collect_skill_tools()` |
...
If a skill declares `allowed-tools`, `collect_skill_tools()` merges those tools into the host job's tool set. Background forks always have at least `DEFAULT_BG_TOOLS`, so skill tools always merge on top. The function deduplicates — if two skills both need `Bash(git *)`, only one entry appears.
