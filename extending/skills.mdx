...
If a skill declares `allowed-tools`, `collect_skill_tools()` merges those tools into the host job's tool set. Background forks always have at least a minimal default tool set, so skill tools always merge successfully. The merger deduplicates tool lists — referencing two skills that both need `Bash(git *)` results in a single entry.
...
This lets skills declare their own tool dependencies. Background forks always have at least a minimal default tool set, so skill tools always merge.
</Accordion>
...
`read_skill()` validates that the resolved skill path is relative to the `skills/` directory before reading. `read_skill()` rejects skill names containing path traversal sequences (e.g., `../../etc/passwd`) with a warning log.
</Accordion>
</AccordionGroup>
