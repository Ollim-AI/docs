...
| `critical` | boolean | No | `false` | Bypasses [ping budget](/scheduling/ping-budget) in background forks |
...
<Tabs>
  <Tab title="Direct actions">
    Direct actions perform a single operation and respond with an ephemeral message. No agent involvement.
...
When the agent sends a button with an `agent:` action, the prompt is persisted to disk so it survives bot restarts. Clicking the button enters an interactive fork where the agent processes the stored prompt.
...
<Steps>
  ...
  <Step title="Agent processes the prompt in a fork">
    The handler enters an interactive fork, sends a fork entry embed with buttons, and streams the agent's response to the channel. The fork follows standard [fork lifecycle](/core-usage/forks) rules including idle timeout.
  </Step>
</Steps>

Inquiry prompts are stored in `~/.ollim-bot/inquiries.json` with a 7-day TTL. Expired entries are cleaned up on read.
...
- **Inquiry storage**: Agent inquiry prompts are written to `~/.ollim-bot/inquiries.json` so the prompt text is available even after a restart. Each entry includes a timestamp for TTL enforcement.
...
- **`critical: true`**: Bypasses the budget. Critical pings are tracked but never blocked.
