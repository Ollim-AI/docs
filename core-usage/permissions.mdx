...
system built on top of the Claude Agent SDK's `canUseTool` callback.
...
| Mode | Behavior |
|------|----------|
| `dontAsk` | Silent denial of non-whitelisted tools. No prompt. **Default.** |
...
silent denial before the SDK ever sees it.
</Note>

## Approval flow

When the permission mode is anything other than `dontAsk`, unapproved tool
use triggers an interactive approval prompt in your Discord DM.
...
<Tabs>
  <Tab title="dontAsk (default)">
    In `dontAsk` mode, the agent works with the tools the SDK whitelists
    automatically. Any tool not in the whitelist or the session-allowed set
    is silently denied — no message appears.
  ...
  <Tab title="default (interactive)">
    In `default` mode, each non-whitelisted tool use produces an approval
    message. You review and react.
