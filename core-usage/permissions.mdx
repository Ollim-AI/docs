...
Background forks use **dynamic gating** for Discord MCP tools — the
`canUseTool` callback checks the fork's `BgForkConfig` flags at runtime:
...
All other tools not in the fork's `allowed_tools` list are denied. The
agent in a background fork cannot trigger approval prompts. Tools
available to background forks are controlled through `allowed-tools` in
the fork's YAML configuration — see
...
Write-protection is enforced at two layers:

1. **SDK hook** — a `PreToolUse` hook (`state_dir_guard`) intercepts
   Write and Edit calls before execution. If the resolved `file_path`
   falls inside `state/`, the hook denies the call with
   `"state/ is write-protected"`. This applies to all contexts — main
   session, interactive forks, and background forks.
2. **Pattern validation** — `tool_policy.py` validates tool patterns at
   startup and at dispatch time. Write/Edit patterns that could match
   the `state/` directory are rejected — jobs declaring patterns like
   `Edit(**)` will not dispatch.

<Note>
The agent can still read files in `state/` but cannot modify them
through its tools. The `PreToolUse` hook catches attempts regardless of
which patterns are declared in `allowed_tools`.
</Note>

## Customizing tool sets

You can extend or override the default tool sets by creating a
`tool-policy.yaml` file in `~/.ollim-bot/`. Changes take effect
without a restart — the file is re-read when its modification time
changes.
