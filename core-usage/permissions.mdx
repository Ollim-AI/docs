...
`default`, `acceptEdits`, and `bypassPermissions` are **fork-scoped** —
changing the mode inside an interactive fork leaves the main session
unchanged.

`dontAsk` is **not** fork-scoped. Switching to or from `dontAsk` inside
an interactive fork affects the main session too.
...
and `bypassPermissions` approves everything — neither triggers the
approval flow below.)
...
Background forks have their own tool restrictions based on the job's
YAML configuration:
