...
2. **Pattern stripping** — `tool_policy.py` validates tool patterns at
   startup and strips any Write/Edit patterns that could match the
   `state/` directory. This means even if a routine or skill declares a
   broad pattern like `Edit(**)`, the pattern is removed before it
   reaches the SDK.
