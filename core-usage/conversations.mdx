...
ollim-bot maintains a single persistent conversation per user. You talk to it through Discord DMs or @mentions in a server channel, and the bot streams its response back in real time. Context carries across messages and survives restarts — every message builds on what came before.
...
<Tabs>
  <Tab title="DMs">
    Send a direct message to the bot. The full message content is forwarded to the agent.
  ...
  <Tab title="@Mentions">
    Mention the bot in any server channel it has access to. The bot strips the mention prefix from the message before forwarding to the agent:

    ```python
    content = message.content.replace(f"<@{bot.user.id}>", "").strip()
    ```

    The bot ignores server messages that do not contain a mention.
  </Tab>
</Tabs>
...
ollim-bot maintains a single persistent session with the Claude Agent SDK. The session ID is stored at `~/.ollim-bot/sessions.json` and is resumed on every bot restart.
...
Session lifecycle events are recorded in `~/.ollim-bot/session_history.jsonl`:
