...
|------|--------|-----|-------------|
| `sessions.json` | Plain string | No | Active Agent SDK session ID |
| `session_history.jsonl` | JSONL | Yes | Session lifecycle events |
| `pending_updates.json` | JSON | No | Background fork updates for main session |
| `inquiries.json` | JSON | No | Button inquiry prompts (7-day TTL) |
