...
| `WEBHOOK_SECRET` | Conditional | — | HMAC secret for webhook authentication; required if `WEBHOOK_PORT` is set |
...
| `fork_messages.json` | JSON | Yes | Fork message references (7-day TTL) |
...
| `token.json` | JSON | Yes | Google OAuth token (auto-generated) |
...
<Tabs>
  ...
  <Tab title="JSONL files">
    ...
    - Lines not starting with `{` and corrupt JSON lines are silently skipped
...
|------|--------|-------------|
| `~/.ollim-bot/credentials.json` | Manual — downloaded from Google Cloud Console | OAuth 2.0 client credentials |
| `~/.ollim-bot/token.json` | Auto-generated on first authentication flow | OAuth refresh and access tokens |
