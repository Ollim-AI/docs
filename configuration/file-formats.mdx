...
| `allowed-tools` | `list[str] \| null` | `null` | Tool allowlist (overrides defaults when set) |
...
<Note>
When `allowed-tools` is not set, background forks receive `MINIMAL_BG_TOOLS` — report, ping, embed, follow-up chain, and help. See [Tool restrictions](/scheduling/background-forks#tool-restrictions) for the full list.
</Note>
