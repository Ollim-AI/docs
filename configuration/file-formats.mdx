...
| `allowed-tools` | `list[str] \| null` | `null` | Additional tools merged with minimal defaults |
...
<Note>
Every background fork always receives `MINIMAL_BG_TOOLS` — report, ping, embed, follow-up chain, and help. When `allowed-tools` is set, your tools are merged in on top of these defaults. See [Tool restrictions](/scheduling/background-forks#tool-restrictions) for details.
</Note>
