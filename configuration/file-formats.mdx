...
| `allowed_tools` | `list[str] \| null` | `null` | Tool allowlist (overrides defaults when set) |
...
<Note>
When `allowed_tools` is not set, background forks receive `MINIMAL_BG_TOOLS` — report, ping, embed, follow-up chain, and help. See [Tool restrictions](/scheduling/background-forks#tool-restrictions) for the full list.
</Note>
...
| `allowed_tools` | `list[str] \| null` | `null` | Tool dependencies merged into the host job's allowed tools |
...
| `thinking_main` | `bool` | `false` | Extended thinking for the main session |
| `thinking_fork` | `bool` | `true` | Extended thinking for interactive forks |
| `max_thinking_tokens` | `int` | `10000` | Token budget when thinking is enabled |
