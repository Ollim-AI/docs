...
└── state/
    ...
    └── routine_sessions/
        └── <routine-id>             # Persistent routine session IDs (one file per routine)
```
...
| `routine_sessions/<id>` | Plain string | Persistent routine session IDs. One file per routine with `session: persistent`. |
...
| `event` | `str` | One of: `created`, `compacted`, `swapped`, `cleared`, `interactive_fork`, `bg_fork`, `isolated_bg`, `persistent_bg` |
...
| `state/routine_sessions/*` | Frequent overwrites on every routine fire |
