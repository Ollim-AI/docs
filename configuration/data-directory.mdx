...
webhooks, spec symlinks, and the optional `tool-policy.yaml`) that the
...
Two markdown files in the data directory root define the bot's identity
and your context. Both are prepended to the system prompt when the bot
starts — see [System prompt](/development/system-prompt#profile-section)
for how they compose into the final prompt.

| File | Bootstrapped | Description |
|------|-------------|-------------|
| `IDENTITY.md` | Yes — created on first run from a built-in template | Bot persona: tone, personality, communication style, and relationship dynamics. Edit this to change how the bot speaks and behaves. |
| `USER.md` | No — you create this yourself | Context about you: work patterns, schedule, ADHD specifics, priorities. The bot and its agent can also edit this file. |

`IDENTITY.md` is written by `bootstrap_identity()` in `profile.py` on
first run. It includes default personality traits (concise, warm,
ADHD-aware, honest pushback) and communication style guidance. If the
file already exists, bootstrapping is skipped — your edits persist
across updates.

`USER.md` is never created automatically. Create it when you want the
bot to have persistent context about you that survives compaction. The
agent can read and edit both files at runtime.
