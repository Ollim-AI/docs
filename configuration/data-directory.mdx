...
`config.json` stores persistent settings configured via [`/config`](/core-usage/slash-commands#config). These
...
| `webhook-spec.md` | Symlink → `docs/webhook-spec.md` | System prompt (webhooks section), webhook handler |
| `skill-spec.md` | Regular file | System prompt (skills section) |
