...
<AccordionGroup>
  <Accordion title="Import your context">
    ...
    > "I put some files from my previous assistant in the imports folder. Can you read
    > through them and pull out anything useful about me — preferences, routines, projects,
    > whatever seems relevant?"

    The bot will extract what matters and fold it into your conversation context. This
    isn't a memory import — it's a conversation where the bot gets to know you faster by
    reading what your last assistant learned.
  </Accordion>
  ...
  <Accordion title="Recreate your automations">
    ...
    directly helps the bot set you up. Browse the [scheduling docs](/scheduling/overview)
    for patterns that match what you had, then ask the bot:

    > "I used to have a morning check-in that reviewed my calendar and tasks. Can you set
    > up a routine like that?"

    The bot reads the same [routine](/scheduling/routines) and
    [reminder](/scheduling/reminders) docs you do — it knows the file formats, frontmatter
    options, and scheduling patterns. Describe what you want in conversation and it creates
    the files. See [real-world examples](/scheduling/examples) for common patterns.
  </Accordion>

  <Accordion title="Extend with MCP tools">
    Missing capabilities you had before — web browsing, file management, code execution?
    ollim-bot supports these through [MCP tools](/extending/mcp-tools) that you configure
    yourself. No marketplace, but full control over what runs and what doesn't.

    See [adding integrations](/extending/adding-integrations) for how to connect new tools,
    and [subagents](/extending/subagents) for delegating specialized tasks.
  </Accordion>
</AccordionGroup>
